...
#rff for testing data
New_in = get_new_in(test_np)


#--------------------Calculate projection length--------------------

New_in_shape = len(New_in[0][1])

K_two = np.zeros((New_in_shape, all_bases.shape[1]))
gauss_gram_two(New_in[0], all_bases, K_two)

New_in= _gds_projection_in(gds,K_two)
...
similarity_all = []
for i in range(New_in[0].shape[1]):
    data_input = K_input_1[:, i]
    similarity_one = []
    for subspace_class_i in New_X:
        length = np.linalg.norm(subspace_class_i[0].T @ data_input, ord = 2)
