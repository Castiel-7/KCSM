...
#parameter-------------------------------------------------------------------------------
sgm = 0.01                                   #Please write the sigma value.
#subdim = 10                                 #Please write the dimensions of class subspace. => Since the following "subdim_info" was added, "subdim" was disabled.
...
subdim_index = []
subdim_c = 0
for subdim_i in subdim_info:
    if subdim_c == 0:
        subdim_index.append(0)
        subdim_index.append(subdim_info[0])
    else:
        subdim_index.append(subdim_index[subdim_c] + subdim_info[subdim_c])
    subdim_c += 1
subdim_index = np.array(subdim_index)
print(subdim_index)
count = 0
...
for train_data in train_x:
    ...
    w, v = w[:rank], v[:, :rank]
    d = v[:, 0:subdim_info[count_class]]                                             #fixed
    K_class.append(d)
    count_class += 1
#-----------------------------------------------------------------------------------------
...
count_k = 0
K_cl = np.zeros((class_index[class_num], np.max(subdim_info)))
'''
...
'''
for i in range(class_num):
    K_cl[class_index[i]:class_index[i+1], 0:subdim_info[i]] = K_class[i]
print(K_cl.shape)


K_D = np.zeros((subdim_index[class_num], subdim_index[class_num]))          #fixed
